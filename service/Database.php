...
{   
    ...
    public function fetch(int $one = 0, $end = true, $column = \PDO::FETCH_ASSOC)
    {
        ...
        $pdo->bindValue(':limit', $this->limit_value, \PDO::PARAM_INT);
        $pdo->bindValue(':offset', $this->offset_value, \PDO::PARAM_INT);
        $pdo->execute();
     ...
     * devided by a comma.
     * @param array $values An array of values in the same order as columns.
     * @param boolean $formula Set it true if you wish to use formula instead of
     * a row value.
     * @return \Database
     * @throws \Exception
     */
    public function update(string $columns, array $values, bool $formula = false): Database
    {
        $this->checkTable();
        $columns = explode(',', $columns);
        try {
